...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  elif model_architecture == 'eurnn':
    return create_eurnn_model(fingerprint_input, model_settings, is_training)
...
def create_eurnn_model(fingerprint_input, model_settings, is_training):
  if is_training:
    dropout_prob = tf.placeholder(tf.float32, name='dropout_prob')
  fingerprint_size = model_settings['fingerprint_size']
  label_count = model_settings['label_count']
  ...
  comp = True
  FFT = False
  input_data = tf.expand_dims(fingerprint_input,-1) 
  cell = EUNNCell(hidden_layer, capacity, FFT, comp)
  ...
  V_init_val = np.sqrt(6.)/np.sqrt(2*fingerprint_size)
  V_weights = tf.get_variable("V_weights", shape = [hidden_layer, label_count],dtype=tf.float32, initializer=tf.random_uniform_initializer(-V_init_val, V_init_val))
  V_bias = tf.get_variable("V_bias", shape=[label_count],dtype=tf.float32, initializer=tf.constant_initializer(0.01))
  hidden_out_list = tf.unstack(hidden_out, axis=1)
  temp_out = tf.matmul(hidden_out_list[-1], V_weights) #tf.stack([tf.matmul(i, V_weights) for i in hidden_out_list])
  logits = tf.nn.bias_add(temp_out, V_bias) #(tf.transpose(temp_out, [1,0,2]), V_bias) 
...
def create_lstm_model(fingerprint_input, model_settings, is_training):
  ...
  input_data = tf.expand_dims(fingerprint_input,-1) 
  cell = tf.nn.rnn_cell.BasicLSTMCell(64, state_is_tuple=True, forget_bias=1)
  hidden_out, _ = tf.nn.dynamic_rnn(cell, input_data, dtype=tf.float32)

  # --- Hidden Layer to Output ----------------------
  V_init_val = np.sqrt(6.)/np.sqrt(2*fingerprint_size)
  V_weights = tf.get_variable("V_weights", shape = [64, label_count],dtype=tf.float32, initializer=tf.random_uniform_initializer(-V_init_val, V_init_val))
  V_bias = tf.get_variable("V_bias", shape=[label_count],dtype=tf.float32, initializer=tf.constant_initializer(0.01))
  hidden_out_list = tf.unstack(hidden_out, axis=1)
  temp_out = tf.matmul(hidden_out_list[-1], V_weights)
  logits = tf.nn.bias_add(temp_out, V_bias)

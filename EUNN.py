...
def _eunn_param(self, hidden_size, capacity=2, fft=False, comp=True):
    ...
    if fft:
        ...
        if capacity_b != capacity_a:
            if comp:
                cos_list_1 = array_ops.concat([
                    cos_list_1,
                    math_ops.complex(
                        tf.zeros([1, hidden_size]), tf.zeros([1, hidden_size]))
                ], 0)
                sin_list_1 = array_ops.concat([
                    sin_list_1,
                    math_ops.complex(
        ...
        diag_vec = tf.reshape(
            tf.concat([cos_list_0, cos_list_1], 1),
            [capacity_a * 2, hidden_size])
        off_vec = tf.reshape(
            tf.concat([sin_list_0, sin_list_1], 1),
            [capacity_a * 2, hidden_size])
...
def _eunn_loop(state, capacity, diag_vec_list, off_vec_list, diag, fft):
    ...
    def layer_tunable(x, i):
        ...
        off = control_flow_ops.cond(
            tf.equal(i % 2, 0), lambda: even_input(off, size),
            lambda: odd_input(off, size))
...
class EUNNCell(keras.layers.Layer):
    ...
    def call(self, inputs, states, scope=None):
        ...
        if self._comp:
            inputs_re = math_ops.matmul(inputs, tf.real(self.kernel))
            inputs_im = math_ops.matmul(inputs, tf.imag(self.kernel))
            inputs = math_ops.complex(inputs_re, inputs_im)
        else:
            inputs = math_ops.matmul(inputs, self.kernel)
